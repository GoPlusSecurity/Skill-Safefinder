...
homepage: https://github.com/GoPlusSecurity/Skill-Safefinder
---
...
## 安装
本 Hook 随 Skill-Safefinder 一起分发。执行 `[AGENT POST-INSTALL HOOK]` 时会自动安装到 `~/.openclaw/hooks/` 目录。

手动安装：
```bash
cp -r hooks/safefinder-interceptor ~/.openclaw/hooks/
openclaw hooks enable safefinder-interceptor
```
